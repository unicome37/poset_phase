...
This is a standard feature of causal set path-sum constructions (see
e.g.~\cite{mathur2021,surya2012}).

That said, $\log|L(P)|$ admits a physical reading as a \emph{measure of
residual ordering freedom}: it counts the number of ways the discrete causal
structure can be ``unfolded'' into a total order. High $|L(P)|$ means the
partial order constrains element ordering weakly --- many orderings are
compatible. Low $|L(P)|$ means the causal structure is rigid, leaving few
compatible orderings. In this sense, $\log|L(P)|$ measures the gap between the
information contained in the partial order and a complete specification of
events.

For manifold-like (Lorentzian) posets, this residual freedom grows moderately
with $N$, reflecting the fact that a causal structure embedded in a
low-dimensional manifold constrains many pairs of events. For KR-like posets,
the 3-layer structure leaves most pairs of elements incomparable, leading to
combinatorially large $|L(P)|$ (as quantified by the Kleitman--Rothschild
theorem~\cite{kleitman1975}). The competition parameter $\gamma$ in our
functional thus controls the trade-off between this combinatorial measure
factor and the geometric penalty $\Pi_{\mathrm{geo}}$: the existence of a
bounded $\gamma_c$ means that a finite geometric action suffices to overcome the
entropy advantage of KR structures.
...
entropy. Whether a deeper connection exists between $|L(P)|$ and physical
entropy in a full causal set quantum gravity theory remains an open question.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Family coverage}: Nine families (including the KR 2-layer and 4-layer controls added post hoc) do not exhaust all posets. Our claim is ``competitive phase among tested structural types,'' not global dominance.
