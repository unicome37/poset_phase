...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Replace dim_proxy_penalty with non-targeted dimension-consistency and estimate gamma_c."
    )
...
if __name__ == "__main__":
    ...
    scale_matched_weight = 8.0 * mean_dim_proxy / mean_dim_consistency if mean_dim_consistency > 0 else 8.0

    variants = {
        "A2_full": lambda row: row["penalty_neutral"] + row["geo_total"],
        "drop_dim_proxy": lambda row: row["penalty_neutral"] + row["geo_total"] - 8.0 * row["geo_dim_proxy_penalty"],
        "width_height_only": lambda row: row["penalty_neutral"] + 2.0 * row["geo_width_height"],
        "replace_dim_with_consistency_raw8": lambda row: (
            row["penalty_neutral"] + row["geo_total"] - 8.0 * row["geo_dim_proxy_penalty"] + 8.0 * row["geo_dim_consistency"]
        ),
        "replace_dim_with_consistency_scale_matched": lambda row: (
            row["penalty_neutral"]
            + row["geo_total"]
            - 8.0 * row["geo_dim_proxy_penalty"]
            + scale_matched_weight * row["geo_dim_consistency"]
        ),
        "width_height_plus_consistency_only": lambda row: (
            row["penalty_neutral"]
                ...
                rows.append(
                    {
                        **row,
                        ...
                        "scale_matched_weight": scale_matched_weight,
    ...
    summary_df = (
        ...
        .agg(
            ...
            mean_dim_proxy=("geo_dim_proxy_penalty", "mean"),
            count=("score", "count"),
        )
        .reset_index()
    ...
    raw_df.to_csv(out_dir / "noncyclic_dim_replacement_raw.csv", index=False, encoding="utf-8-sig")
    summary_df.to_csv(out_dir / "noncyclic_dim_replacement_summary.csv", index=False, encoding="utf-8-sig")
    report_df.to_csv(out_dir / "noncyclic_dim_replacement_gamma_c_report.csv", index=False, encoding="utf-8-sig")

    print((out_dir / "noncyclic_dim_replacement_gamma_c_report.csv").as_posix())
    print()
    print(f"scale_matched_weight={scale_matched_weight:.6f}")
    print()
