...
在参考结构的邻域内，椭球近似是精确的。其精度由实验验证：Lor4D在所有测试N值（16–1024）上排名#1，margin从 $S_{\mathrm{MD}} \approx 3$（N=16）增长到 $1.93 \times 10^8$（N=1024），呈幂律发散。
...
**实验设计**：10个独立随机种子 × 8个N值 (16–128) × 20 reps/seed = 27,200个独立样本。
...
Mahalanobis的唯一失败发生在 seed=1001, N=16，margin仅0.6（对比N=128平均margin=40.4）。LSD-Well在最危险的配置（N=16, seed=3141）下margin=0.003，极其微弱但仍然正确。
...
6%的Mahalanobis失败全部集中在N=16，且失败时Lor4D仍排名#2（仅被Lor5D超过，margin极小）。保留率 > 95% 意味着训练/测试切分几乎不影响分离度——判别力是结构性的，不是过拟合的。

### 6.3 第三重：N=16失败根因分析

N=16是整个实验方阵中唯一出现Lor4D非#1（在Mahalanobis CV模式下）的配置。根因诊断结果：
...
在N=16时，两者的有效维度重叠严重（差距仅0.4，而单个样本的 $\sigma(d) \approx 0.38$）。这是 **物理分辨率极限**，不是统计方法的缺陷：在仅16个时空点的因果集中，区分4维和5维是一个本征困难的问题。
...
**结论**：N=16的"失败"实际上证实了理论的自洽——它标记了离散化分辨率的物理边界，而非统计方法的内在不稳定性。
...
| 5 | **Mahalanobis LSD** | `mahalanobis_lsd_test.py` | 零参数，全 N #1，3-seed 鲁棒 |
...
| 14 | **N=16 根因诊断** | `n16_mahalanobis_cv_rootcause.py` | 唯一入侵者 = Lor5D (19/19)，物理分辨率极限 |
...
| 0d | Prediction B 修订 | `prediction_b_lsd_well_revision.py` | N≥20 时 Lor4D #1/17，margin 单调增长 |
