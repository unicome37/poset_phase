...
(viii) **Sequential screening simulation.** Using independent penalty strengths $\gamma_1$ (geometric pre-filter) and $\gamma_2$ (information-theoretic selection), a two-step protocol—A2 at soft $\gamma_1$ followed by A4 at strong $\gamma_2$ within the TOP-$K$ shortlist—achieves Lor4D rank \#1 at *all three* tested $N$ values ($N = 10, 20, 40$). At $N = 40$, where no single penalty mode ever reaches \#1, the sequential protocol (with $\gamma_1 = 0.5$, $K = 5$, $\gamma_2 = 2.0$) uniquely selects Lor4D from a 17-family library. This quantitatively validates the layered architecture: the geometric layer provides "permissive admission" (retaining Lor4D at moderate penalty) while the information-theoretic layer provides "precise selection" (uniquely choosing Lor4D from the geometric shortlist).

(ix) **S\_MD path integral integration.** Embedding the Mahalanobis identity functional $S_{\mathrm{MD}}$ directly into the Boltzmann action ($A = -\log H + \alpha \cdot S_{\mathrm{MD}}$) yields Lor4D rank \#1 at all $N$ with $\alpha \geq 0.5$, even without any structural penalty. Adding $S_{\mathrm{MD}}$ rescues Lor4D's ranking under conditions where information-theoretic penalties alone degrade (e.g. $\gamma = 2.0$: \#3 → \#1). This confirms that $S_{\mathrm{MD}}$ encodes irreducible identity information independent of the entropy–penalty trade-off, though the inherent circularity (reference manifold presupposes Lor4D) limits its role to a second-layer discriminator rather than a stand-alone selector.
