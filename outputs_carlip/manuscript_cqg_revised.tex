...
$\gamma_c(N)$ scan with exact linear-extension counts in the confirmatory window $N=10$--$44$.
...
\begin{enumerate}[nosep]
\item We construct an 8-family ensemble with exact linear-extension counts ($N = 10$--$44$) and
...
We define $H(P) = \log |L(P)|$ where $L(P)$ is the set of linear extensions, and use it as a combinatorial complexity proxy.
...
measure factor and can modify which families dominate. Our goal here is more
modest: to analyze finite-$N$ competition in the labeled (or label-sampling)
setting in which linear extensions are used as a combinatorial proxy.
...
\begin{table}[h]
\centering
\caption{Critical coupling $\gamma_c$ under $A_2$ (exact linear-extension counts). All values
...
Beyond $N = 44$, we use an asymmetric mixed protocol: exact linear-extension counts for
...
\begin{enumerate}[nosep]
\item \textbf{Bounded phase transition.} Under $A_2$ with exact linear-extension counts, the
...
\subsection{Combinatorial Meaning of $\log|L(P)|$}

$\log|L(P)|$ is the logarithm of the number of total orders compatible with
the partial order. In plain terms, it measures how many full event orderings
remain possible once the causal constraints are fixed. High $|L(P)|$ means the
partial order leaves more compatible orderings; low $|L(P)|$ means the causal
structure is more rigid. We therefore use $\log|L(P)|$ as a combinatorial
complexity proxy for residual ordering freedom, not as a claim about physical
entropy.
...
\textit{Universe} \textbf{5}(3), 83 (2019).
arXiv:1705.05417.
...
\textit{J.\ Math.\ Phys.} \textbf{19}(8), 1711--1713 (1978).
DOI: 10.1063/1.523869.

\bibitem{promel2001}
H.~J.~Pr{\"o}mel, A.~Steger, and A.~Taraz,
``Counting Partial Orders with a Fixed Number of Comparable Pairs,''
\textit{Combinatorics, Probability and Computing} \textbf{10}(2), 159--177 (2001).
...
\textit{Class.\ Quantum Grav.} \textbf{38}, 045017 (2021).
arXiv:2009.07623.
