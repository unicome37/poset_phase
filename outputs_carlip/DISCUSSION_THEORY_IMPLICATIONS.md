...
## §9 FLRW κ=1.0 边界风险（可直接粘贴 Discussion）

### 9.1 English paragraph (paper-ready)

> **Background-dependent robustness and the FLRW boundary.**
> The current curved-background falsification results support a background-dependent, rather than uniform, robustness claim. In split low-$N$ tests ($N=256,512$), de Sitter and weak-field Schwarzschild satisfy the top-2 criterion, while FLRW at $\kappa=1.0$ reaches the pre-registered hard-fail threshold. In completed high-$N$ runs ($N=768,1024$; 5/5 seeds per branch), FLRW at $\kappa=1.0$ shows partial recovery (failure ratio $0.3<0.5$), i.e., instability remains but no hard fail is triggered under the same rule. We therefore interpret FLRW $\kappa=1.0$ as a boundary-sensitive regime of the current flat-centered local-basin model, not as evidence that the two-layer framework fails globally. The safest statement is that curvature robustness is validated for de Sitter-like and weak-field Schwarzschild conditions in the tested window, while FLRW near $\kappa=1.0$ remains a controlled boundary case requiring dedicated metric-faithful follow-up.

### 9.2 中文工作稿（内部对照）

> 当前曲率结果应表述为“分背景稳健”，而非“统一弱曲率稳健”。在 split low-$N$（$N=256,512$）中，de Sitter 与 weak-field Schwarzschild 保持 top-2，但 FLRW 在 $\kappa=1.0$ 触及预注册 hard-fail 门槛；在 high-$N$（$N=768,1024$，三路均 5/5 完成）中，FLRW $\kappa=1.0$ 出现部分回升（fail ratio $0.3<0.5$），但仍表现边界敏感。故应将 FLRW $\kappa=1.0$ 定位为当前 flat-centered local-basin 模型的边界区，而非两层框架的全局失效证据。安全口径是：de Sitter-like 与 weak-field Schwarzschild 在测试窗口内稳健，FLRW $\kappa\approx1.0$ 需作为条件性边界单独处理并继续做 metric-faithful 验证。
