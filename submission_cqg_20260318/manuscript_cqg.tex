...
% (This submission package is self-contained; figures live under this folder.)
...
entropy $H = \log|L(P)|$ (the number of natural labelings / linear extensions),
the partition function is overwhelmingly dominated by
...
causal set evolution. Reviews by Carlip~\cite{carlip2017}, Surya~\cite{surya2019},
Henson~\cite{henson2006}, and Dowker~\cite{dowker2005} summarize related
developments, including the KR-dominance issue in labeled ensembles.
In the path-sum setting, Loomis and Carlip~\cite{loomis2018} gave analytic
evidence that suitable weighting can suppress non-manifoldlike orders.
More broadly, phase-transition phenomena in random poset ensembles have a
long combinatorial history (e.g.\ Dhar~\cite{dhar1978} and Pr\"omel
et~al.~\cite{promel2001}), and the ranked/layered structures that dominate
counting measures are well studied.
In this paper, we use $\log|L(P)|$ as a \emph{labeled measure term} for finite
poset ensembles; we do \emph{not} interpret it as thermodynamic ``matter
entropy,'' and we explicitly separate measure effects from the action terms we
introduce.

However, no prior work has demonstrated an explicit, finite-size
...
samples from ten structurally distinct families organized in two tiers:
...
$[0.15, 1.00]$ for $N \geq 12$). This provides finite-size support for a
...
Work on causal set dynamics and phase structure spans both CST and earlier
combinatorial studies of random partial orders. In CST, the BD
action~\cite{benincasa2010,benincasa2011} provides a continuum-motivated
discrete action, and numerical evidence for continuum behavior and finite-size
scaling in 2D has been reported (e.g.\ Surya~\cite{surya2012} and
Glaser et~al.~\cite{glaser2018}). Loomis and Carlip~\cite{loomis2018} analyzed the
suppression of non-manifoldlike sets in the causal set path integral from an
analytic perspective. Separately, phase transitions in random poset ensembles
have been studied since at least Dhar~\cite{dhar1978}, with later results on the
typical structure of finite posets and related transition behavior (e.g.\
Pr\"omel et~al.~\cite{promel2001}).

Our contribution is to provide (i) a finite-$N$ $\gamma_c(N)$ curve computed
with exact linear extension counts over a wide confirmatory range, and (ii) a
systematic ablation/replace pipeline that isolates which order-theoretic
constraints are actually responsible for the observed bounded-threshold
behavior. Cunningham and Surya~\cite{cunningham2020} investigated dimensionally
restricted causal set models; our ``dimensional consistency'' setup instead
asks which dimensionality is \emph{selected} within a mixed-dimensional ensemble.
...
\bibitem{dhar1978}
D.~Dhar,
``A combinatorial approach to phase transitions in partially ordered sets,''
\textit{J.\ Math.\ Phys.} \textbf{19}, 171 (1978).

\bibitem{promel2001}
H.~J.~Pr\"omel \textit{et al.},
``Phase transition and typical structure in random partial orders,''
