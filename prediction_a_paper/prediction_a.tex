...
Carlip \cite{Carlip2024} showed that suppression of non-manifold-like (Kleitman--Rothschild)
sets in the causal-set path integral comes from the link term in the action, with higher-order
terms not critical for that problem. Loomis and Carlip \cite{Loomis2018} established the
...
\end{equation}
This normalized variant tests whether density-normalization changes the qualitative behavior
of the link action.
...
Carlip \cite{Carlip2024} showed that for the suppression of KR-like layered sets,
``the suppression comes from the link term in the action; higher-order terms are not
critical for this problem but are needed to recover GR/locality.''
...
\begin{itemize}
  ...
  \item The CV of $13.9\%$ (generator) and $17.7\%$ (size) are low but not negligible;
    they may decrease with larger $N$ or broader generator ensembles,
    or may plateau at a finite value reflecting genuine $O(1)$ corrections.
...
\begin{enumerate}
  \item \textbf{Figure~1.} Winner heatmap: Link action vs BDG $d{=}4$ standard across
    $(N, \lambda)$ grid.
  \item \textbf{Figure~2.} BDG $d{=}4$ component breakdown (stacked bar) + score comparison
    at $\lambda = 6$ for representative $N$.
  \item \textbf{Figure~3.} 4D margin of victory scaling with $N$ under link action
    (vs 2D, vs 3D, vs 5D).
  ...
  \item \textbf{Figure~5.} Causal interval profile ($C_0, C_1, C_2, C_3$) by dimension
    at $N = 68$.
  \item \textbf{Figure~6.} (Core figure) $\Xi$ strip plot across transitions and generators.
  \item \textbf{Figure~7.} $\Xi_{4 \to 5}$ vs $N$ for each generator,
    showing convergence across sizes and geometries.
  \item \textbf{Figure~8.} Asymmetric barrier ratio $\Xi_{4 \to 5} / \Xi_{3 \to 4}$ vs $N$.
  \item \textbf{Figure~9.} Large-$N$ winner heatmap: $(N, \lambda)$ grid from $N = 20$ to $N = 112$.
  \item \textbf{Figure~10.} $\Xi_{4 \to 5}$ stability across $N = 20$--$112$.
  \item \textbf{Figure~11.} Link-density crossover.
  \item \textbf{Figure~12.} (Master derivation figure) Four-panel figure:
    $C_0/N$ power-law fits, $\log H/N$ log-linear fits, gap decomposition, predicted vs numerical $\Xi$.
  \item \textbf{Figure~13.} $\Xi$ decomposition: numerator, denominator, and ratio for all transitions.
  \item \textbf{Figure~14.} Ordering fraction $p_d$ and decay rate from Monte Carlo.
  \item \textbf{Figure~15.} (Predictive validation) Parameter extrapolation to $d = 6, 7$.
  \item \textbf{Figure~16.} $\Xi$ staircase: blind predictions vs numerical measurements.
  \item \textbf{Figure~17.} Observable comparison at $\lambda = 8$ for $d = 5, 6, 7$.
\end{enumerate}
