...
The present work concerns finite causal-set ensembles rather than a controlled continuum limit.
...
A geometrically motivated semi-analytic derivation from Minkowski geometry yields
$\Xi_{4 \to 5} = 11.31$, within $0.4\%$ of the numerical median,
with no additional fitted parameter once the closure coefficient
$\varepsilon = \pi/48$ is fixed.
Blind extrapolation to higher dimensions yields measured values
$\Xi_{5 \to 6} \approx 46$ and $\Xi_{6 \to 7} \approx 38$,
supporting the picture that barriers intensify beyond 4D.
This asymmetric barrier structure makes $3{+}1$ dimensions a \textbf{favored finite-ensemble regime} of the
