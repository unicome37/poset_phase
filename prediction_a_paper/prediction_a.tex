...
We emphasize that $\ln|L(G)|$ is used here as a \emph{combinatorial measure term}: for an unlabeled poset~$G$, the number of natural labelings consistent with the order equals the number of linear extensions, so any labeled path sum implicitly weights $G$ by $|L(G)|$. This is a choice of ensemble and measure, not a claim that $\ln|L(G)|$ is a unique or physically correct definition of gravitational entropy.
...
the Boltzmann/Poisson occupancy form therefore motivates setting $\alpha = 1$
without introducing an additional scale parameter.
...
\end{equation}
The relative error is only $0.4\%$, within the geometric closure
$\varepsilon = \pi/48$.
...
Table~\ref{tab:xi_prediction} compares this geometric closure against
...
providing a first out-of-sample check of Eq.~\eqref{eq:xi_scaling_closed_form}.
