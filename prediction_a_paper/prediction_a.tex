...
\emph{Within this framework, 3+1 dimensions are favored not because 4D is intrinsically optimal,
but because the structural price of crossing from 4D to 5D --- measured in link-penalty units
per entropy unit --- is anomalously high.}
...
The key result (Table~\ref{tab:xi_generators}, Figure~\ref{fig:xi_core}): \textbf{$\Xi_{4 \to 5} \approx 10$ across all three generators},
...
(Figures~\ref{fig:xi_convergence} and~\ref{fig:xi_asymmetry}). This transforms $\Xi_{4 \to 5}$ from a diagnostic
...
Within-$N$ analysis across dimensions reveals a striking anti-correlation
...
In more physical terms: \textbf{within the present finite-ensemble setting, $3{+}1$ dimensions
...
\textbf{the link term also governs dimensional selection},
...
\begin{quote}
\textbf{Within the present finite-ensemble setting, the data support a picture in which
$3{+}1$ dimensions are favored by an asymmetric dimensional barrier in the link sector of
causal dynamics. The effective link-penalty/entropy ratio at the $4{\to}5$ boundary
...
drift --- suggesting that the structural price of the $4{\to}5$ transition is an intensive,
size-independent property of causal poset link dynamics. The continuum-limit implications
of this finite-ensemble result remain an open question for future work.}
\end{quote}
