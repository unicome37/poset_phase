...
def add_variant(raw_df: pd.DataFrame, variant: str) -> pd.DataFrame:
    df = raw_df.copy()
    if variant == "switch_raw":
        df["switch_variant"] = df["cg_family_switch_rate"]
    elif variant == "switch_weighted":
        df["switch_variant"] = 1.5 * df["cg_family_switch_rate"]
    elif variant == "switch_zscore":
        pieces = []
        for n, sub in df.groupby("n"):
            mean = float(sub["cg_family_switch_rate"].mean())
            std = float(sub["cg_family_switch_rate"].std(ddof=0))
            std = std if std > 1e-12 else 1.0
            sub = sub.copy()
            sub["switch_variant"] = (sub["cg_family_switch_rate"] - mean) / std
            pieces.append(sub)
...
def build_scan(raw_df: pd.DataFrame, zetas: list[float], variant: str) -> pd.DataFrame:
    raw_df = add_variant(raw_df, variant)
    rows: list[dict] = []
    for zeta in zetas:
        df = raw_df.copy()
        df["score_switch"] = df["score_A2_gamma"] + zeta * df["switch_variant"]
        summary = (
            df.groupby(["n", "family"])
            .agg(
                mean_score_switch=("score_switch", "mean"),
                std_score_switch=("score_switch", "std"),
                mean_switch_variant=("switch_variant", "mean"),
...
def plot_scan(scan_df: pd.DataFrame, out_path: Path) -> None:
    ...
    for ax, (n, sub) in zip(axes[:, 0], scan_df.groupby("n")):
        for (variant, family), fam_df in sub.groupby(["variant", "family"]):
            ...
            ax.plot(fam_df["zeta"], fam_df["mean_score_switch"], marker="o", label=f"{family} [{variant}]")
        ax.set_title(f"Controlled-window A2 + zeta*switch, N={n}")
        ax.set_xlabel("zeta")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Zeta scan using cg_family_switch_rate only.")
    parser.add_argument("--config", default="config_pairwise_switch_only_scan.yaml", help="Path to YAML config.")
    return parser
...
if __name__ == "__main__":
    ...
    scan_df.to_csv(out_dir / "pairwise_switch_scan.csv", index=False, encoding="utf-8-sig")
    cross_df.to_csv(out_dir / "pairwise_switch_crossings.csv", index=False, encoding="utf-8-sig")
    plot_scan(scan_df, out_dir / "pairwise_switch_scan.png")

    print((out_dir / "pairwise_switch_scan.csv").as_posix())
    print((out_dir / "pairwise_switch_crossings.csv").as_posix())
    print((out_dir / "pairwise_switch_scan.png").as_posix())
    print()
