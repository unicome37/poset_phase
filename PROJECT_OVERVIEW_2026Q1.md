...
| **D — 两层筛选** | Mahalanobis 零参数选择 Lor4D？ | ✅ 完成 | 撰写中 | **Carlip 后核心方向**，25 族 N=16-1024 |
...
Mahalanobis 距离，以 Lor4D 参考流形 $\boldsymbol{\mu}(N)$ 为中心，零自由参数。在所有 N 和全部 25 族中唯一将 Lor4D 排名第 1。
...
| 结论 | 来源 | 关键数据 |
|------|------|---------|
| Mahalanobis 零参数全 #1 | mahalanobis_lsd_test | N≥20, 25 族, CV 100%, seed 80/80 |
| Basin deepening | basin_deepening_experiment | gap: 1.38→9.4×10⁷; V_eff∝N⁻¹·⁵⁷ |
| 参考流形坍缩 | mu_trajectory_theory | det(Σ)∝N⁻³·³⁸, σ²∝N⁻¹ (CLT) |
| d_eff→4 收敛 | cstar_wstar_first_principles | μ_d(∞)=3.967≈4 |
| 对抗鲁棒性 | expanded_family_robustness | 25 家族全 N Lor4D #1 |
...
| 风险 | 状态 | 缓解 |
|------|------|------|
| N=16 Mahalanobis 可能排 Lor5D 第 1 | ✅ 已解决 | REPS=80 turn-on 实验：N≥14 全部 10/10 rank#1，N=12 为物理分辨极限 (6/10) |
...
| 周次 | 任务 | 交付物 |
|------|------|--------|
| Week 1 | ✅ 锁定声明边界 + 解决小 N 问题 | **N≥14 turn-on 边界确认** (远优于预期 N≥20) |
